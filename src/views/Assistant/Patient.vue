		...
		<template>
			<el-tabs v-model="activeName" >
				<el-tab-pane label="个人档案" name="first">
                  ...
                  <el-button type="primary" @click.native="dialogVisible = false">返回</el-button>
                  </el-form>
				</el-tab-pane>
...
      	},
		...
		xiangqing:function(index,row){
			...
			this.assessmentForm.userId = row.userId
			this.assessmentForm.userName = row.userName
			this.userId = row.userId
			this.healthGuidanceForm.userId = row.userId
			...
			//查询医疗记录
			this.medicalRecordPage.columnFilters = {userId: {name:'userId', value:row.userId}}
			this.findMedicalRecord(this.medicalRecordPage)
			this.dialogVisible = true;
			//查询健康指导
			this.findHealthGuidance(row.userId)
			//查询健康日志
			this.healthLogPage.columnFilters = {userId: {name:'userId', value:row.userId}}
			this.findHealthLog(this.healthLogPage)
			this.assessmentResult.columnFilters = {userId: {name:'userId', value:row.userId}}
			this.findAssessment(this.assessmentResult)

			//查询保健品电子药单
			this.chufangPage.columnFilters = {recordId: {name:'recordId', value:row.userId}}
			this.findChufang(this.chufangPage)
		},
